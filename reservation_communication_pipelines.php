...
function reservation_communication_affiche_enfants($flux) {
	if ($e = trouver_objet_exec($flux['args']['exec'])
		...
		if (autoriser('creercommunicationdans', 'rubrique', $id_rubrique)) {
			$bouton .= icone_verticale(_T("communication:icone_creer_communication"), generer_url_ecrire("communication_edit", "id_rubrique=$id_rubrique"), "communication-24.png", "new", "right")
					. "<br class='nettoyeur' />";
		}

		$flux['data'] .= $lister_objets('communications', array('titre'=>_T('communication:titre_communications_rubrique') , 'id_rubrique'=>$id_rubrique, 'par'=>'titre'));
		$flux['data'] .= $bouton;
